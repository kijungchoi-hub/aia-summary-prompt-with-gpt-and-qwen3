...
핵심 목표는 현업 피드백을 프롬프트 본문에 계속 누적하지 않고, `고정 규칙`, `가변 설정`, `학습 규칙`, `자체검증`으로 분리해 안정적으로 운영하는 것입니다.
...
- `docs/evaluation`
  - 요약 품질 평가 기준
- `docs/analysis`
  - 실제 STT 요약 데이터 분석 결과, 샘플 기반 현업 피드백 대응표
- `prompts`
...
5. 생성 직전 `self-check`로 누락과 과다 반영을 점검
...
5. 실행 전에는 `prompts/execution_prompt_template.md`의 `self-check` 항목까지 포함해 조립하는 것이 좋습니다.

## 현업 피드백 반영 절차

1. 현업 코멘트를 먼저 `누락`, `과다`, `순서`, `용어`, `근거성` 중 하나로 분류합니다.
2. 반복되는 피드백은 `examples/feedback_rules_expected_ops.json`의 `feedback_rules` 또는 `analysis_hints`에 반영합니다.
3. 프롬프트 본문 수정이 필요한지 보기 전에 `feedback_rules`와 `self-check` 보강으로 해결 가능한지 먼저 확인합니다.
